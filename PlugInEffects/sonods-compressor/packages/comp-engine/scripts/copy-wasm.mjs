...
const candidates = [
  path.resolve(__dirname, '../../comp-core/target/wasm32-unknown-unknown/release/comp_core.wasm'),
  path.resolve(__dirname, '../../../target/wasm32-unknown-unknown/release/comp_core.wasm'),
];
...
fs.writeFileSync(path.join(destDir, 'comp_core.wasm'), wasmBytes);

const base64Wasm = wasmBytes.toString('base64');
const tsContent = `// Auto-generated from comp_core.wasm
...
console.log(`✅ Bundled comp_core.wasm (${(wasmBytes.length / 1024).toFixed(1)} KB) into wasmBinary.ts`);
