...
const candidates = [
  path.resolve(__dirname, '../../dsp-core/target/wasm32-unknown-unknown/release/gate_dsp_core.wasm'),
  path.resolve(__dirname, '../../../target/wasm32-unknown-unknown/release/gate_dsp_core.wasm'),
];
...
fs.writeFileSync(path.join(destDir, 'gate_dsp_core.wasm'), wasmBytes);

const base64Wasm = wasmBytes.toString('base64');
const tsContent = `// Auto-generated from gate_dsp_core.wasm
...
console.log(`✅ Bundled gate_dsp_core.wasm (${(wasmBytes.length / 1024).toFixed(1)} KB) into wasmBinary.ts`);
